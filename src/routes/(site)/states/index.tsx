...
export default function StatesIndexPage() {
  ...
  const visibleStates = createMemo(() => {
    ...
    switch (sortMode()) {
      case "state":
        return [...filtered].sort((left, right) => left.state.localeCompare(right.state));
      case "reviewed":
        ...
        <div class="mb-6 flex flex-wrap gap-3">
          <label class="data-chip text-[0.72rem]">
            ...
            <select
              ...
              onInput={(event) => setProposalFocusFilter(event.currentTarget.value)}
            >
        ...
        <div class="grid gap-4 lg:grid-cols-2">
          {visibleStates().map((state) => (
            <StateCard
              ...
              significance={state.manifest?.shortNote ?? "Published registry entry."}
